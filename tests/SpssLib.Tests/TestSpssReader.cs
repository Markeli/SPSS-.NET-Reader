...
namespace Test.SpssLib
{
    public class TestSpssReader
    {
            ...
            try
            {
                ReadData(fileStream, out varCount, out rowCount, 
                    new Dictionary<int, Action<int, Variable>>
                    {
                        {0, (i, variable) =>
                        {
                            Assert.Equal("varaible ñ", variable.Label);
                            Assert.Equal(DataType.Numeric, variable.Type);
                        }},
                        {1, (i, variable) =>
                        {
                            Assert.Equal("straße", variable.Label);
                            Assert.Equal(DataType.Text, variable.Type);
                        }}
                    },
                    new Dictionary<int, Action<int, int, Variable, object>>
                    {
                        ...
                        {   // All numeric values are doubles
                            Assert.Equal(value.GetType(), typeof(double));
                            double v = (double) value;
                            Assert.Equal(varenieValues[r], v);
                        }},
                        {1, (r, c, variable, value) =>
                        {
                            Assert.Equal(value.GetType(), typeof(string));
                            string v = (string) value;
                            Assert.Equal(streetValues[r], v);
                        }}
                    });
            }
            finally
        ...
        public void TestReadMissingValuesAsNull()
        {
            ...
            {
                Assert.Equal(varValues[c][r], value);
            };
